...
class Game {
	...
	nextBidEmit(player,bidwinner,currentbid,firstbid,log,biddouble,delay){
		var d = {'player':this.playerFromNumber(player),
	...
	bidOver(player,log) {
		...
		var d = {'winner':player,'bidvalues':teambids,'biddouble':this.biddouble,'biddoubleteam':this.biddoubleteam,'log':log};
		...
		if(this.trump_open && this.marriage_not_d){
			for(var i=0;i<4;i++){
				...
				if(this.checkM(this.cards[i],this.trump_card)){
					...
					var d = {'player':this.playerFromNumber(i),'bidvalues':teambids,'biddouble':this.biddouble,'biddoubleteam':this.biddoubleteam,'delay':delay};
	...
	trumpOpenReceive(player){
		...
		var d = {'player':this.playerFromNumber(this.trump_opener),'card':this.trump_card,'operation':'open'};
	...
	trumpSetReceive(player,suit,log){
		...
		var d = {'player':this.playerFromNumber(this.trump_setter),'card':this.trump_card,'operation':'set'};
		this.emitlog[0].push('trump');
		this.emitlog[1].push(d);
		io.in(this.roomID).emit('trump', d);
